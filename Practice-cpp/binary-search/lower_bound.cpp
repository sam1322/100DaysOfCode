...
int lowerbound(vi &a,int start ,int end ,int target){
	if(start>=end)
		return start ;

	int mid = start + (end - start)/2 ; 
	if(a[mid]>=target)
		return lowerbound(a,start,mid,target)  ; 
	else {
		return lowerbound(a,mid+1,end,target) ;
	}
...
void solve(){
	...
	printf("position of %d is %d \n",target,Lowerbound(a,0,a.size(),target)) ;
}
