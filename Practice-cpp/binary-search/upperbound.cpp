...
int upperbound(vi &a,int start ,int end ,int target){
	if(start>=end)
		return end ;

	int mid = start + (end - start )/2 ; 
	// cout<<"Start "<<start<<" End "<<end<<" Mid "<<mid<<" "<<a[mid]<<" "<<target<<endl;
	if(a[mid]>target)
		return upperbound(a,start,mid,target)  ; 
	else {
		return upperbound(a,mid+1,end,target) ;
	}
...
void solve(){
	...
	printf("position of %d is %d \n",target,upperbound(a,0,a.size(),target)) ;
}
